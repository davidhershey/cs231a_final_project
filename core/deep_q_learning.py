...
class DQN(QN):
    ...
    def initialize(self):
        ...
        print(os.path.join(self.config.model_output, "model.ckpt"))
        ckpt = tf.train.get_checkpoint_state(self.config.model_output)
        if ckpt:
          saver = tf.train.import_meta_graph(os.path.join(self.config.model_output,"model.ckpt.meta"))
    ...
    def save(self):
        ...
        self.saver.save(self.sess, os.path.join(self.config.model_output, "model.ckpt"))
