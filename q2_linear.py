...
class Linear(DQN):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope,reuse=reuse) as sc:
            # if reuse:
            #     sc.reuse_variables()
            sf = tf.contrib.layers.flatten(state)
            W =  tf.get_variable("weights", (sf.get_shape()[1],num_actions),
                                initializer=tf.random_normal_initializer())
            b = tf.get_variable("biases", (num_actions),
                        initializer=tf.constant_initializer(0.0))
            out = tf.matmul(sf,W)+b
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        targets = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        varc = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        nameToV = {}
        ops = []
        for v in varc:
            nameToV[v.name] = v
        for v_t in targets:
            v_name = q_scope + v_t.name[len(target_q_scope):]
            v = nameToV[v_name]
            ops.append(tf.assign(v_t,v))
            # print "assigning ", v.name, " to ",v_t.name
        # grouped = tf.no_op()
        # for op in ops:
        #     grouped = tf.group(grouped,op)
        # op = tf.Print(op,[v_t],first_n=2,summarize=16)
        # # grouped = tf.tuple(ops)
    ...
    def add_loss_op(self, q, target_q):
        ...
        future = self.config.gamma * tf.reduce_max(target_q,axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        gradients = opt.compute_gradients(self.loss,train_vars)
        clipped_gradients = []
        ...
        self.train_op = opt.apply_gradients(zip(clipped_gradients, train_vars))
        ##############################################################
        ######################## END YOUR CODE #######################
